...
const SUPABASE_PROJECT_CONFIGS = {
    'kdbtusugpqboxsaosaci': { // معرف المشروع 1 (الافتراضي)
        ...
        serviceRoleKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtkYnR1c3VncHFib3hzYW9zYWNpIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1Mjg1NDU1NCwiZXhwIjoyMDY4NDMwNTU0fQ.AQKVeRlWPoXmplNRg_xKL1OMPP-TW8qCGUcftTYaky8" // **تأكد من تحديث هذا المفتاح السري**
    },
...
app.get('/api/user/:userId/chats', async (req, res) => {
    ...
    try {
        ...
        console.log('DEBUG: بيانات محادثات المستخدم التي يتم إرسالها (أول محادثة):', JSON.stringify(userChats.slice(0, 1)));
...
app.get('/api/group/:groupId/members', async (req, res) => {
    ...
    try {
        ...
        console.log('DEBUG: بيانات أعضاء المجموعة التي يتم إرسالها (أول عضو):', JSON.stringify(membersInfo.slice(0, 1)));
