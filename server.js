...
app.get('/api/posts/followed/:userId', async (req, res) => {
    ...
    try {
        ...
        followedUsersIds.push(userId); // **التعديل هنا: إضافة معرف المستخدم الحالي**
...
// نقطة نهاية للحصول على منشورات مستخدم معين (لصفحته الشخصية مثلاً)
app.get('/api/user/:userId/posts', async (req, res) => {
    const { userId } = req.params;
    try {
        const baseQuery = `WHERE p.author_id = $1`;
        const postsWithDetails = await getPostsWithDetails(baseQuery, [userId], userId);
        res.status(200).json(postsWithDetails);
    } catch (error) {
        console.error('ERROR: Failed to get user posts:', error);
        res.status(500).json({ error: 'فشل جلب منشورات المستخدم.' });
    }
});
